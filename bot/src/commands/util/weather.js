  ...
  async exec() {
    ...
    if (locationRes.status !== 'OK') {
      switch (locationRes.status) {
        case 'ZERO_RESULTS':
          return this.response.error('No results found');
        case 'REQUEST_DENIED':
          return this.response.error('Request denied');
        case 'INVALID_REQUEST':
          return this.response.error('Invalid request');
        case 'OVER_QUERY_LIMIT':
          return this.response.error('Over limit');
        case 'UNKNOWN_ERROR':
          return this.response.error('An unkown error has occurred');
        default:
          return this.response.error('I got no fucking clue what just happened, but it borke.');
      }
    }

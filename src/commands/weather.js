    ...
    request(geocodeOptions).then((locationRes) => {
        if(locationRes.status !== "OK")
        {
            switch(locationRes.status){
                case "ZERO_RESULTS":
                    return msg.channel.sendMessage("No results found")
                case "REQUEST_DENIED":
                    return msg.channel.sendMessage("Request denied")
                case "INVALID_REQUEST":
                    return msg.channel.sendMessage("Invalid request")
                case "OVER_QUERY_LIMIT":
                    return msg.channel.sendMessage("Over limit")
                case "UNKNOWN_ERROR":
                    return msg.channel.sendMessage("An unkown error has occured")
            }
        }
        else{
            let city;
